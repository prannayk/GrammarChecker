...
type Aring = [Atom]
data Production = Produce Aring [Aring] deriving (Show,Eq)
...
rmLeftRecursion [] = []
rmLeftRecursion (prod:prods) = (selfRecurPr prod (rmLeft prod (rmLeftRecursion prods)))
...
getAlternate dones head (ringo:rango) = [Produce head (map (++rango) (match [ringo] ([],[],dones)))]
...
selfRecur :: Production -> Bool
selfRecur (Produce ring rings) = and $ rings >>= (\x -> [ring == (take (length ring) x)])

selfRecurPr :: Production -> [Production] -> [Production]
selfRecurPr prod dones
	| some = []
	| otherwise = rmSelfRecur prod dones
	where some = selfRecur prod

rmSelfRecur :: Production -> [Production] -> [Production]
rmSelfRecur (Produce prod rings) dones = ((filter (\(Produce x _) -> x == prod) dones) >>= useToRmSR (Produce prod rings)) ++ dones

useToRmSR (Produce prod rest) (Produce done stuff) = [(Produce prod (map (\x -> x ++ prod ++ [(NonTerm "'")]) stuff)),(Produce (prod ++ [(NonTerm "'")]) (rest >>= doSome prod))]

doSome prod rest = if (prod == (take (length prod) rest)) then [(drop (length prod) rest) ++ prod ++ [(NonTerm "'")],[Empty]] else [rest]